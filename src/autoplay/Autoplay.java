...
public class Autoplay {
    public static void main(String args[]) {
        ...
        try {
            ProcessBuilder server_pb = new ProcessBuilder("java", "-cp", "bin/production/game", "boardgame.Server","-ng", "-k");
            server_pb.redirectOutput(ProcessBuilder.Redirect.INHERIT);

            Process server = server_pb.start();

            //One need to change those variables
            ProcessBuilder client1_pb = new ProcessBuilder("java", "-cp", "bin/production/game", "-Xms520m", "-Xmx520m",
                    "boardgame.Client", "student_player.OtherStudent");
            client1_pb.redirectOutput(ProcessBuilder.Redirect.INHERIT);

            ProcessBuilder client2_pb = new ProcessBuilder("java", "-cp", "bin/production/game", "-Xms520m", "-Xmx520m",
